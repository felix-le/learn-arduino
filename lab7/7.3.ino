...
pinMode(buzzerPin, OUTPUT); //Setup red LED pin as an output pin. pinMode(button1Pin, INPUT); //Setup button1 pin as an input pin.
 ...
 randomSeed(analogRead(0));// setup function the random number generator 
}
void loop() { //The loop function runs forever.
  if (digitalRead(button1Pin) == HIGH) { 
    for(int i =0; i<sizeof(notes)/sizeof(notes[0]); i++){
    	colour = random(9,12); //random number between 9,10,11)
      tone(buzzerPin, notes[i], duration[1]);
...
tone(buzzerPin, NOTE_B4,408); delay(408);
tone(buzzerPin, NOTE_A4,408); delay(408);
tone(buzzerPin, NOTE_G4,408); delay(408);
tone(buzzerPin, NOTE_A4,408); delay(408);
tone(buzzerPin, NOTE_B4,408); delay(408);
tone(buzzerPin, NOTE_B4,408); delay(408);
